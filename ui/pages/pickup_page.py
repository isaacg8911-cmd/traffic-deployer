...
def build_pickup_page(win) -> QWidget:
    ...
    v.addWidget(b_sec)
    card_counter = QWidget()
    card_counter.setObjectName("pickupCounterCard")
    cv = QVBoxLayout(card_counter)
    cv.setContentsMargins(10, 10, 10, 10)
    cv.addWidget(win._h("PicoCount study"))
    hint_pc = QLabel("Pull count data from the USB counter for the selected installed site.")
    hint_pc.setObjectName("hint")
    hint_pc.setWordWrap(True)
    cv.addWidget(hint_pc)
