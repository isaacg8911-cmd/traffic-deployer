...
# --- CORE CONFIG ---
st.set_page_config(page_title="Live Wire V51.46 Dual-Node List", layout="centered")
    ...
    for cfg in est_configs:
        ...
        for sid, (ex_lat, ex_lon, street) in excel_map.items():
            if sid in raw_map:
                ...
                # STICK TO BEG AND END ONLY
                nodes = [(ex_lat, ex_lon), (m_lat, m_lon)]
...
if not st.session_state.optimized_route:
    ...
    if ex and maps and st.button("🚀 SYNC DUAL-NODE ROUTE"):
        process_data([{"file": f, "label": f"Day {i+1}"} for i, f in enumerate(maps)], ex)
else:
    st.title("📁 Manifest Control")
    view_mode = st.radio("Select View:", ["Interactive Map", "List View"], horizontal=True)
    ...
    if view_mode == "Interactive Map":
        ...
        for i, s in enumerate(st.session_state.optimized_route):
            ...
            list_data.append({
                ...
                "Site ID": sd['id'],
                "Street": sd['street'],
                "Status": "✅ Done" if sd['Installed'] else "📍 Pending"
            })
        st.table(list_data)
    ...
    if idx < len(st.session_state.optimized_route):
        ...
        if idx > 0 and st.button("⬅️ PREV STOP"):
            st.session_state.current_index -= 1; auto_save(); st.rerun()
    else:
        st.success("🏁 All sites installed.")
        st.link_button("🏠 RETURN TO GARDEN GROVE", f"https://www.google.com/maps/search/?api=1&query={HOME_COORDS[0]},{HOME_COORDS[1]}")
