...
if "site_data" not in st.session_state:
    st.session_state.site_data = {} # Holds all the Excel data
...
if mode == "📍 1. Installation":
                    ...
                    for site in temp_route:
                        if site['id'] not in st.session_state.site_data:
                            st.session_state.site_data[site['id']] = {
                                ...
                                "LAT": site['lat'], "LON": site['lon'] # Saved for records
                            }
                    st.rerun()
    ...
    # Only show sites that were actually installed
    ...
    if not installed_sites:
        ...
        # Build the exact CSV structure you requested
        export_df = pd.DataFrame(installed_sites)
        # Reorder columns to match your exact requested format
