...
# --- ROCK-SOLID CONFIG ---
st.set_page_config(page_title="Live Wire V51.35 Manifest-Map", layout="centered")
...
def process_data(est_configs, excel_files):
    ...
    if not final_raw:
        st.error("No valid site matches found. Check your California coordinates.")
        return
...
if not st.session_state.optimized_route:
    ...
    # --- NEW: MANIFESTED MAP VIEW ---
    st.subheader("Day Manifest Map")
    map_data = []
    for s in st.session_state.optimized_route:
        sd = st.session_state.site_data.get(s['uid'])
        if sd:
            color = "#00FF00" if sd['Installed'] else "#FFA500"
            map_data.append({"lat": sd['lat'], "lon": sd['lon'], "color": color})
    
    if map_data:
        st.map(pd.DataFrame(map_data), color="color")
    ...
    if 0 <= idx < len(st.session_state.optimized_route):
        ...
        if sd:
            st.info(f"**STOP {idx+1} of {len(st.session_state.optimized_route)}**\n\nSITE: {sd['id']}\n\nSTREET: {sd['street']}")
